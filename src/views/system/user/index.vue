  ...
  <div class="app-container">
        ...
        <pane size="84">
          <el-col>
            <el-form
              ...
              label-width="100px"
            >
              <el-form-item label="教师手机号" prop="userName">
                <el-input
    ...
    <el-dialog :title="title" v-model="open" width="40%" append-to-body>
      <el-form :model="form" :rules="rules" ref="userRef" label-width="120px">
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="班级" v-if="form.roleIds === 2">
              <el-select
...
import { getClassList } from "@/api/system/report";
import { pwdEncrypt } from "@/utils/jsencrypt";
    ...
    .$prompt('请输入"' + row.userName + '"的新密码', "提示", {
      ...
      inputValidator: (value) => {
        ...
        if (!pattern.test(value)) {
          return "需要包含大小写字母、数字和特殊字符";
        }
        return true;
      },
    })
    .then(({ value }) => {
      resetUserPwd(row.userId, pwdEncrypt(value)).then((response) => {
...
function importTemplate() {
  proxy.download(
    ...
    {},
    `教师账号管理模版_${new Date().getTime()}.xlsx`
  );
}
...
function handleUpdate(row) {
  ...
  getUser(userId).then((response) => {
    ...
    form.value.roleIds =
      response.roleIds && response.roleIds.length && response.roleIds[0];
    open.value = true;
    title.value = "修改";
    form.password = "";
  });
